...
model = dict(
    ...
    backbone=dict(
        ...
        depth_backbone_cfg=dict(
            ...
            # init_cfg=dict(type='Pretrained', checkpoint='torchvision://resnet50'),
            style='pytorch',
            in_channels=1  # Depth 입력용
        ),
    ),
    neck=dict(
        type='FPN',
        in_channels=[256 * 2, 512 * 2, 1024 * 2, 2048 * 2],  # RGB와 Depth 채널 합치기
        out_channels=256,
        start_level=1,
...
data_root = '/home/rise/Desktop/graduation/CalibNet/datasets/CUSTOM/'
...
train_pipeline = [
    ...
    dict(type='Collect', keys=['img_rgb', 'img_depth', 'gt_bboxes', 'gt_labels', 'gt_masks'])
]
test_pipeline = [
    dict(type='LoadRGBDImages'),
    dict(
        ...
        transforms=[
            ...
            dict(type='Pad', size_divisor=32),
            dict(type='ImageToTensor', keys=['img_rgb', 'img_depth']),
            dict(type='Collect', keys=['img_rgb', 'img_depth'])
        ])
]
...
log_level = 'INFO'
work_dir = './work_dirs/sipmask_rgbdr50_PhotoMetricDistortionRGB_CosineAnnealing_ensenso_test/'
load_from = None
resume_from = None
workflow = [('train', 1)]
