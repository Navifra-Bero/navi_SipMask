...
class Normalize:
    ...
    Args:
        mean (sequence): Mean values for 3 channels.
        std (sequence): Std values for 3 channels.
        to_rgb (bool): Convert BGR to RGB if True.
    """
    ...
    def __call__(self, results):
        """Normalize the RGB image."""
        # Specify 'img_rgb' to avoid applying on depth
        for key in ['img_rgb']:
            if key in results:
...
class NormalizeDepth:
    """Normalize the depth image to a fixed range.

    Args:
        to_float32 (bool): Convert depth image to float32 if True.
    """
    ...
    def normalize(self, img):
        ...
        img_max = np.max(img)
        if img_min == img_max:
            return np.ones(img.shape) * 127.5 if img_max != 0 else img
        return np.clip((img - img_min) / (img_max - img_min), 0, 1) * 255
    
    def __call__(self, results):
        """Normalize the depth image."""
        img_depth = results.get('img_depth')
        
        if img_depth is None:
            raise ValueError("Depth image 'img_depth' not found in results.")
...
@PIPELINES.register_module()
class NormalizeRGBD:
    """Normalize both RGB and Depth images separately, and concatenate them.

    Args:
        mean_rgb (sequence): Mean values for RGB channels.
        std_rgb (sequence): Std values for RGB channels.
        mean_depth (sequence): Mean values for Depth channel.
        std_depth (sequence): Std values for Depth channel.
        to_rgb (bool): Whether to convert RGB images from BGR to RGB.
    """

    def __init__(self, mean_rgb, std_rgb, mean_depth, std_depth, to_rgb=True):
        self.mean_rgb = np.array(mean_rgb, dtype=np.float32)
        self.std_rgb = np.array(std_rgb, dtype=np.float32)
        self.mean_depth = np.array(mean_depth, dtype=np.float32)
        self.std_depth = np.array(std_depth, dtype=np.float32)
        self.to_rgb = to_rgb

    def __call__(self, results):
        """Normalize RGB and Depth images separately, then concatenate.

        Args:
            results (dict): Result dict from the pipeline.

        Returns:
            dict: The updated result dict with normalized images.
        """
        # Normalize RGB image
        if 'img_rgb' in results:
            results['img_rgb'] = mmcv.imnormalize(
                results['img_rgb'], self.mean_rgb, self.std_rgb, to_rgb=self.to_rgb
            )

        # Normalize Depth image
        if 'img_depth' in results:
            results['img_depth'] = mmcv.imnormalize(
                results['img_depth'], self.mean_depth, self.std_depth, to_rgb=False
            )

        # Concatenate RGB and Depth images to form a 4-channel image
        if 'img_rgb' in results and 'img_depth' in results:
            # Check if img_rgb and img_depth have compatible dimensions
            assert results['img_rgb'].shape[:2] == results['img_depth'].shape[:2], \
                "RGB and Depth image dimensions must match."

            # Expand depth image to match RGB dimensions
            img_depth_expanded = np.expand_dims(results['img_depth'], axis=2)
            results['img'] = np.concatenate((results['img_rgb'], img_depth_expanded), axis=2)

        # Update the normalization config
        results['img_norm_cfg'] = dict(
            mean_rgb=self.mean_rgb, std_rgb=self.std_rgb,
            mean_depth=self.mean_depth, std_depth=self.std_depth,
            to_rgb=self.to_rgb
        )
        return results
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(mean_rgb={self.mean_rgb}, std_rgb={self.std_rgb}, '
        repr_str += f'mean_depth={self.mean_depth}, std_depth={self.std_depth}, to_rgb={self.to_rgb})'
        return repr_str
...
class ConcatRGBD:
    ...
    Args:
        keys (list): List of keys to concatenate. Should include 'img_rgb' and 'img_depth'.
        output_key (str): The key for the concatenated output image. Defaults to 'img'.
    """

    def __init__(self, keys=['img_rgb', 'img_depth'], output_key='img'):
        self.keys = keys
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Updated result dict with concatenated image.
        """
        img_rgb = results[self.keys[0]]
        img_depth = results[self.keys[1]]

        # Depth 이미지는 1채널이므로 차원을 맞춰줍니다 (H, W) -> (H, W, 1)
        if img_depth.ndim == 2:
            img_depth = img_depth[:, :, None]

        # RGB와 Depth를 채널 차원에서 결합 (H, W, 3) + (H, W, 1) -> (H, W, 4)
        img_rgbd = np.concatenate((img_rgb, img_depth), axis=2).astype(np.float32)
        
        # 결과에 결합된 이미지를 추가
        results[self.output_key] = img_rgbd
        return results
...
class PhotoMetricDistortionRGB:
    """Apply photometric distortion to the RGB image only.
    ...
    def __call__(self, results):
        """Apply distortion to the RGB image only, leaving Depth unchanged."""

        if 'img_rgb' not in results:
            raise KeyError("Expected 'img_rgb' in results for RGB image processing.")
        
        img_rgb = results['img_rgb'].astype(np.float32)
        ...
        # Random contrast before or after HSV conversion
        mode = random.randint(2)
        if mode == 1 and random.randint(2):
            alpha = random.uniform(self.contrast_lower, self.contrast_upper)
            img_rgb *= alpha

        # Convert to HSV for further distortion
        img_rgb = mmcv.bgr2hsv(img_rgb)

        # Random saturation
        if random.randint(2):
            img_rgb[..., 1] *= random.uniform(self.saturation_lower, self.saturation_upper)

        # Random hue
        if random.randint(2):
            img_rgb[..., 0] += random.uniform(-self.hue_delta, self.hue_delta)
            img_rgb[..., 0][img_rgb[..., 0] > 360] -= 360
            img_rgb[..., 0][img_rgb[..., 0] < 0] += 360

        # Convert back to BGR
        img_rgb = mmcv.hsv2bgr(img_rgb)

        # Random contrast if not applied before
        ...
        if random.randint(2):
            img_rgb = img_rgb[..., random.permutation(3)]
